...
See my website for a more general description of this function: [Creating Python Package Layers in AWS Lambda using only…AWS Lambda?](https://nedcharles.com/software/Python_Package_Layers_in_AWS_Lambda.html)
...
In this Lambda function, I only download the module specified at the input.  This means that all required dependencies are not downloaded along with the module, and will need to be created separately.  For example, scikit-learn requires the modules numpy, scipy, and [joblib](https://pypi.org/project/joblib/).  To be able to use scikit-learn code, I created a layer for scikit-learn, a layer for joblib, and used the AWS-provided numpy/scipy layer.  In the Lambda function that will run scikit-learn code, you then need to add these three layers in the console to your function, and order them by dependency with numpy/scipy first, joblib second, and finally scikit-learn.  

**Code Overview**

After reading in the module name passed into the function (see Inputs below), the Python code checks the inputs for errors, and then creates two new directories in the /tmp folder for downloading the wheel file from PyPI and setting up a cache folder.  Once these folders are setup, the function calls pip through the subprocess function, passing in the locations of the two /tmp folders above.  These folders need to be specified to pip, otherwise, it attempts to download the archive file (wheel/zip) to the user home directory "~", which it doesn't have permission to access.  

After successfully downloading the appropriate wheel file for the specified module, the function then repackages the file contents such that the layer zip file will load them into /opt/python/lib/python3.8/site-packages (3.8 is the version number and will change for a different Python runtime version).  Once that is complete, the Lambda function will publish the new Lambda layer using the boto3 function [publish_layer_version](https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/lambda.html?highlight=lambda#Lambda.Client.publish_layer_version)  If the function finishes successfully, your new layer should be listed in the AWS Lambda Layers section in the console.
...
"logs:PutLogEvents",  
"s3:PutObject" (optional if you want to put your layer code archive in S3)
...
**Features for Future**
- Adding custom tags to the Lambda layer at creation time
- Adding the SHA256 hash of the original wheel file
- Adding an unzipped package size as a tag to help in calculation of total layer size (see limitation above)
